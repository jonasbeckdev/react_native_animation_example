...
import {popG22} from '../redux/actions'

class G22 extends Component {
    id = 'g22'
    render() {
                    ...
                    this.props.table.cards.map((card, i)=>{
                        return (
                            ...
                            <Card
                                ...
                                releaseOn={(to, yes, no)=>{
                                    if (ground.g22.cards.length <= 2) {
                                        no()
                                        return
                                    }
                                    this.props.releaseOn(ground.g22, to, yes, no)
                                }}
                                setDragging={()=>{
                                    this.props.setDragging(this.id)
                                }}
                                moveOn={(to)=>{
                                    return logicRlease(ground.g22, to)
                                }}
...
export default connect(mapStateToProps, {popG22})(G22);
