...
export function cardSize() {
    ...
    const size = width;
    //calculate padding
    return {width: Math.floor(size/(columnCnt+2) - 4), height: Math.floor(size/columnCnt - 4)}
}

export const percentUnit = Math.floor(100/columnCnt)
export const percentRemind = 100-Math.floor(100/columnCnt)*columnCnt

export function createDeck() {
    let suite = ['c', 's', 'h', 'd'];
    let numbers = ['a', '2', '3', '4', '5', '6', '7', '8', '9'];// let numbers = ['a', '2', '3', '4', '5', '6', '7', '8', '9', '10', 'j', 'q', 'k'];
    let deck = [];
    ...
    for(i = 0; i < suite.length ; i++) {
        for(j = 0; j < numbers.length; j++) {
            ...
            deck.push({id:id, faceDown: true, portrait: true});
        }
    }
    return deck;
}

export function createRoyal() {
    let suite = ['s', 'h'];
    let numbers = ['j', 'q', 'k'];// let numbers = ['a', '2', '3', '4', '5', '6', '7', '8', '9', '10', 'j', 'q', 'k'];
    let deck = [];
...
export function cardtoNumber(card) {
    ...
    return parseInt(id.substring(0, 1)=='a'?'1':id.substring(0, 1))
}
