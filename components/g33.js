...
import {popG33} from '../redux/actions'

class G33 extends Component {
    id = 'g33'
    render() {
                    ...
                    this.props.table.cards.map((card, i)=>{
                        return (
                            ...
                            <Card
                                ...
                                releaseOn={(to, yes, no)=>{
                                    if (ground.g33.cards.length <= 2) {
                                        no()
                                        return
                                    }
                                    this.props.releaseOn(ground.g33, to, yes, no)
                                }}
                                setDragging={()=>{
                                    this.props.setDragging(this.id)
                                }}
                                moveOn={(to)=>{
                                    return logicRlease(ground.g33, to)
                                }}
...
export default connect(mapStateToProps, {popG33})(G33);
