...
import {popG11} from '../redux/actions'

class G11 extends React.Component {
    id = 'g11'
    render() {
                    ...
                    this.props.table.cards.map((card, i)=>{
                        return (
                            ...
                            <Card
                                ...
                                releaseOn={(to, yes, no)=>{
                                    if (ground.g11.cards.length <= 2) {
                                        no()
                                        return
                                    }
                                    this.props.releaseOn(ground.g11, to, yes, no)
                                }}
                                setDragging={()=>{
                                    this.props.setDragging(this.id)
                                }}
                                moveOn={(to)=>{
                                    return logicRlease(ground.g11, to)
                                }}
...
export default connect(mapStateToProps, {popG11})(G11);
